...
%%% from an experimental session that had no injection administration.
...
%%% In this case all events are marked as injectionless.
%%% Function returns events table.

function events = n00_make_injectionless_events(session_info, recording_list, recording_system, configurations)

%%% Initialize empty output in case there are no valid events created
events = [];
...
%%% stimulus, because ultimately that is how classifications would be
%%% carried out
...
%%% Get actual recording starts and end times relative to the start of the
%%% first recording
...
if event_count > 0
    ...
    %%% Same number of columns as injection events in case the events were
    %%% to be merged for classification regardless of whether there was an injection event
    ...
    injectionless          = ~false_array;
    is_even                = false_array;
    is_ketamine            = false_array;
    injection_administered = false_array;

function n00_create_subject_configuration(varargin)
if isempty(varargin)
    ...
    n_channels = 128;
    included_banks = {'A', 'B'};
    stimulation_ROI = {'Angular Gyrus', 'Posterior Cingulate Cortex', 'Supramarginal Gyrus', 'MTG', 'MFG', 'SFG'};
    micro_labels = {};
    optimized_stim_parameters = {'amplitude', 'location'};
else
    root_directory = varargin{1};
    username = varargin{2};
    analysis_folder_name = varargin{3};
    subject = varargin{4};
    n_channels = varargin{5};
    included_banks = varargin{6};
    stimulation_ROI = varargin{7};
    micro_labels = varargin{8};
    optimized_stim_parameters = varargin{9};
end
...
subjects_directory = fullfile(root_directory, 'shared/lega_ansir/subjFiles');
configurations_directory = fullfile(analysis_directory, 'configurations');
todays_date = datestr(now, 'yyyymmdd');
save_directory = fullfile(configurations_directory, subject, todays_date);
if ~isfolder(save_directory)
    mkdir(save_directory);
end

%%% Verify that jacksheet and depth_el_info have been created
jacksheet_file = dir(fullfile(subjects_directory, subject, 'docs/*jacksheet*.txt'));
...
if ~isempty(jacksheet_file)
    ...
    if isempty(jacksheet_file)
        error('Subject %s has no jacksheet on docs folder.', subject)
    end
...
depth_el_info_file = dir(fullfile(subjects_directory, subject, 'docs/*depth_el_info*.txt'));
if ~isempty(depth_el_info_file)

    depth_el_info_file = fullfile({depth_el_info_file.folder}, {depth_el_info_file.name});
    depth_el_info_file(contains(depth_el_info_file, {'~', '._'})) = [];
    
    if isempty(depth_el_info_file)
        error('Subject %s has no depth_el_info on docs folder.', subject)
    end
...
subject_electrodes = n00_get_subject_electrodes(session);

%%% Process the electrode_list
[subject_electrodes, bad_electrodes, removed_electrodes] = n03_filter_electrode_list(root_directory, username, analysis_folder_name, 'both', false, subject_electrodes, 'single');
...
%%% Save these tables
save(fullfile(save_directory, 'subject_electrodes.mat'), 'subject_electrodes');
save(fullfile(save_directory, 'bad_electrodes.mat'), 'bad_electrodes');
save(fullfile(save_directory, 'removed_electrodes.mat'), 'removed_electrodes');

monopolar_file = sprintf('%s_%s_mono_L0M0.csv', subject, todays_date);
bipolar_file = strrep(monopolar_file, 'mono', 'bi');
stimulation_file = sprintf('%s_%s_stimulation_locations.csv', subject, todays_date);
...
writecell(monopolar_list, fullfile(save_directory, monopolar_file));

bipolar_list = make_bipolar_list(subject_electrodes);
writecell(bipolar_list, fullfile(save_directory, bipolar_file));

bipolar_list = bipolar_list(2:end, :);

stimulation_labels = subject_electrodes.label(contains(subject_electrodes.neurologist_location, stimulation_ROI));
...
if ~has_stimulation_targets
    ...
    writecell(stimulation_pairs, fullfile(save_directory, stimulation_file));
end
...
for idx = 1:length(elemem_experiments)
    ...
    if has_stimulation
        template_file_name = fullfile(save_directory, sprintf('%s_%s_%s_%s.json', subject, experiment_name, '%s', todays_date));
    else
        configuration_file = fullfile(save_directory, sprintf('%s_%s_%s.json', subject, experiment_name, todays_date));
    end
